...
export const AmountField = ({ id, signalValue, decimals = 18 }: AmountFieldProps) => {
	const inputRef = useRef<HTMLInputElement>(null)

	const displayValue = useComputed(() => {
		if (signalValue.value === undefined) return ''
		const floatValue = parseFloat(formatUnits(signalValue.value, decimals))
	...
	function updateValue(e: JSX.TargetedEvent<HTMLInputElement, Event>) {
		...
		signalValue.value = inputValue.length ? parseUnits(inputValue, decimals) : undefined
	}

	function clearValue() {
		signalValue.value = undefined
	...
	return (
		<div class='relative grid grid-cols-[1fr,min-content] grid-rows-2 grid-flow-col-dense gap-x-3 py-3 px-4'>
			<label for={id} class='text-sm text-white/50'>Amount</label>
			<input ref={inputRef} required id={id} placeholder="1.00" value={displayValue.value} inputMode='numeric' pattern='^\s*(\d+(\.\d+)?)?\s*$' onInput={updateValue} class='h-6 bg-transparent outline-none placeholder:text-white/20 peer' />
			...
			<div class='row-span-2 flex items-center justify-center peer-placeholder-shown:hidden'>
				<button type='button' onClick={clearValue} class='w-8 h-8 flex items-center justify-center text-sm outline-none focus:border focus:border-white'><Icon.Xmark /></button>
			</div>
