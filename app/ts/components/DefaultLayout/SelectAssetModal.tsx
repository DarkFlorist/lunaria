...
export const SelectAssetModal = (props: SelectAssetModalProps) => {
	const count = useSignal<number>(6)
	...
	return (
		<div class='fixed inset-0 overflow-y-scroll scrollbar-hidden'>
			...
			<Dialog count={count.value}>
				<div class='sticky top-0 py-4 z-10'>
					<div class='text-3xl font-bold mt-8 mb-4'>Select Token</div>
					<input class='h-12 w-full px-4 bg-black border border-white/50 outline-none' type="number" min={1} value={count.value} onInput={event => count.value = parseInt(event.currentTarget.value)} />
				</div>
				<TokenGrid count={count.value}>
					{Array.from({ length: count.value }, () => (
						<TokenCard onSelect={() => props.onClose()} onClose={() => props.onClose()} />
					))}
					<AddTokenCard />
				</TokenGrid>
			</Dialog>

			<button class='fixed top-4 right-4 text-3xl z-10' onClick={() => props.onClose()}>&times;</button>
		</div>
	)
}
...
const TokenCard = (props: TokenCardProps) => {
	...
	return (
		<div class='aspect-square bg-neutral-800 p-4 flex items-end relative group outline-none' tabIndex={1}>
			<div class='relative'>
				<div class='bg-white/20 w-12 h-12 rounded-full mb-2' />
				<div class='font-bold'>Dai (DAI)</div>
					...
					<>
						<div class='absolute inset-0 w-full flex items-center justify-center bg-black/80 border border-white/50'>
							<div class='text-center p-4'>
								...
								<button class='border border-white/50 px-3 py-1' onClick={() => removal.value = false}>Yes</button>
							</div>
						</div>
						<button class='absolute top-4 right-4' onClick={() => removal.value = false}>
							<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13.5 2.116 8.616 7l4.884 4.884a1.146 1.146 0 0 1 0 1.616 1.146 1.146 0 0 1-1.616 0L7 8.616 2.116 13.5a1.146 1.146 0 0 1-1.616 0 1.146 1.146 0 0 1 0-1.616L5.384 7 .5 2.116A1.146 1.146 0 0 1 .5.5a1.146 1.146 0 0 1 1.616 0L7 5.384 11.884.5A1.146 1.146 0 0 1 13.5.5a1.146 1.146 0 0 1 0 1.616Z" fill="#fff" /></svg>
						</button>
					</>
				) : (
					<>
						<button class='absolute inset-0 w-full flex items-center justify-center bg-black/80 border border-white/50' onClick={() => props.onSelect()}>Tap to use</button>
		...
		<div class='aspect-square bg-neutral-900 p-4 flex items-center justify-center'>
			<div class='grid place-items-center'>
				<div class='bg-black/20 w-20 h-20 rounded-full mb-2 flex items-center justify-center'>
					<svg class='text-white/50' width="3em" height="3em" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M8 2.75a.5.5 0 0 0-1 0V7H2.75a.5.5 0 0 0 0 1H7v4.25a.5.5 0 0 0 1 0V8h4.25a.5.5 0 0 0 0-1H8V2.75Z" fill="currentColor" /></svg>
				</div>
