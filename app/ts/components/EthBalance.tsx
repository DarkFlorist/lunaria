...
export const EthBalance = () => {
	...
	switch (wallet.status) {
		...
		case 'connected':
			// assign new key on account change to update balance
			return <Connected key={wallet.account} fetchFn={wallet.getBalance} />
	}
}
...
const Disconnected = ({ connectFn }: { connectFn: () => void }) => {
	return (
		<div class='flex flex-col items-center md:items-start gap-1 mb-6'>
			...
			<div>
				<button class='font-bold underline-offset-4 hover:underline focus:underline focus:outline-none' onClick={connectFn}>
					Connect now
				</button>{' '}
...
const Wrapper = ({ children }: { children: ComponentChildren }) => {
	return <div class='flex flex-col items-center md:items-start gap-1 mb-6'>{children}</div>
}

const RefreshButton = ({ onClick }: { onClick: () => void }) => {
	return (
		<button type='button' class='focus:outline-none focus:bg-white/10 focus:ring-1 focus:ring-white rounded' title='Refresh' onClick={onClick}>
			<Icon.Refresh />
		</button>
	)
}
