...
const AddTokenDialog = ({ children }) => {
    ...
    return (_jsx("dialog", { ref: ref, class: 'w-full text-white backdrop:bg-black/80 backdrop:backdrop-blur-[2px] max-w-full max-h-full md:max-w-fit md:max-h-[calc(100vh-3rem)] md:max-w-fit bg-transparent', onSubmit: handleDialogSubmit, children: children }));
};
...
const QueryStatus = () => {
    ...
    switch (tokenQuery.value.state) {
        ...
        case 'resolved':
            ...
            return (_jsxs("div", { class: 'px-4 py-3 border border-dashed border-white/30 grid grid-cols-1 gap-y-2', children: [_jsx("div", { class: 'text-white/50 text-sm', children: "Found a matching address" }), _jsxs("div", { class: 'grid grid-cols-1 gap-y-1', children: [_jsxs("div", { children: [_jsx("span", { class: 'font-bold', children: token.name }), " ", _jsxs("span", { class: 'text-white/50', children: ["(", token.symbol, ")"] })] }), _jsx("pre", { class: 'px-3 py-2 border border-white/10 font-mono bg-white/10 text-white/80 text-sm', children: token.address })] }), _jsx(AddressChecksumWarning, { show: token.address !== address.value })] }));
    }
};
const AddressChecksumWarning = ({ show }) => {
    if (!show)
        return _jsx(_Fragment, {});
    return (_jsxs(_Fragment, { children: [_jsx("p", { class: 'text-amber-500 text-sm', children: "You entered an address that does not have the correct checksum and Lunaria tried to fetch the address without it." }), _jsxs("label", { class: 'flex items-center gap-x-2 text-sm', children: [_jsx("input", { type: 'checkbox', required: true }), " I've verified the result is indeed the token contract I want to add."] })] }));
};
const TokenDataToFields = () => {
    ...
    return (_jsxs(_Fragment, { children: [Object.keys(token).map(key => _jsx("input", { type: 'hidden', name: key, value: token[key] })), _jsxs("button", { type: 'submit', class: 'px-4 py-3 border border-white/50 hover:bg-white/10 hover:border-white text-center outline-none flex gap-x-1 items-center justify-center', children: [_jsx(PlusIcon, {}), tokenExistsInCache.value ? '' : 'Save and ', "Use"] })] }));
};
