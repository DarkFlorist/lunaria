...
export const TransferResult = () => {
    ...
    const recipientChecksumValid = useComputed(() => {
        ...
        if (inputAddress.match(/^(0x)?[0-9a-fA-F]{40}$/)) {
            if (getAddress(inputAddress.toLowerCase()) !== inputAddress && inputAddress.toLowerCase() !== inputAddress)
                return false;
        }
        return true;
    });
    ...
    switch (transaction.value.state) {
        case 'inactive':
            ...
            if (!recipientChecksumValid.value) {
                return _jsx(ConfirmField, { title: 'Warning: Invalid Address Checksum', description: 'The recipient address does not pass the checksum validation. Please check the address entered has the correct letter-casing.', label: 'I understand the risk and want to continue anyway.' });
            }
